...
struct gpio_key  {
	...
	struct work_struct work;		// 每一个按键都哟一个work
};
...
// 实现内核线程处理函数
// handle是传入的最后一个参数
static irqreturn_t gpio_key_thread_fn(int irq, void *handle)
{
	// handle->gpio
	struct gpio_key *gpio_key = handle;
	int val;
	val = gpiod_get_value(gpio_key->gpiod);
	// 打印内核线程名称和id
	printk("gpio_key_thread_fn: the process is %s pid %d\n",current->comm, current->pid);	
	printk("gpio_key_thread_fn key %d %d\n", gpio_key->gpio, val);	
...
static int gpio_keys_probe(struct platform_device* pdev)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		// 初始化工作队列
		INIT_WORK(&p_gpio_keys[i].work, key_work_func);
	}
	for (i = 0; i < count; i++)
	{
		...
		// 修改成request_threaded_irq
		request_threaded_irq(p_gpio_keys[i].irq, gpio_key_irq_winter, gpio_key_thread_fn, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "winter_gpio_key", &p_gpio_keys[i]);
	}
...
int gpio_keys_remove(struct platform_device* pdev)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		cancel_work_sync(&p_gpio_keys[i].work);
		flush_work(&p_gpio_keys[i].work);
	}
	// 释放空间
	kfree(p_gpio_keys);
	return 0;
